...
const RestaurantSettings = () => {
  ...
  useEffect(() => {
    if (restaurant) setBars(restaurant?.kitchenZone.bars);
  ...
  const doApiEditInfo = async (_dataBody) => {
    let url =
      API_URL + "/restaurants/editRest/" + localStorage.getItem(RESTAURNAT_ID);
    try {
  ...
  return (
    <div className="flex-auto  py-7 px-4 sm:px-6 lg:px-8">
      ...
      <div className="w-full max-w-md ">
        ...
        <div className="flex justify-evenly my-3 ">
          <button
            className="border-2 rounded-full  bg-slate-300 p-2"
            onClick={() => {
              setIsGallery(false);
            }}
            >
          ...
          {!isGallery && (
            ...
            className="border-2 rounded-full  bg-slate-300 p-2"
            onClick={() => {
              if (isChangeAddress) setIsChangeAddress(false);
              else setIsChangeAddress(true);
            }}
            >
              change addres
            </button>
      ...
      {!isGallery && restaurant && (
        ...
        className="mt-3 space-y-5 w-full max-w-md"
        action="#"
        method="POST"
        >
              ...
              {isChangeAddress && restaurant?.address && (
                <>
                  ...
                  <InputStreetNumber
                    ...
                    errors={errors}
                    defaultValue={
                      restaurant.address.num != null
                      ? restaurant.address.num
                      : "street number ..."
                    }
                    />
                </>
      ...
      {isGallery && (
        <Gallery
        images={restaurant?.gallery?.img}
        videos={restaurant?.gallery?.video}
        display={display}
        />
        )}
        </div>
